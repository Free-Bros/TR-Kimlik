SWEP.PrintName = "İşletme Belgesi Kurumsal"
	...
	if !IsValid(vm) then return end
		...
		if (m) then
			...
			cam.Start3D2D(pos+ang:Right()*3+ang:Forward()*-1.80+ang:Up()*1.0, ang, 0.02)
			        draw.SimpleText("İşletme Sahibi:", "PassHud2", 40, -50, Color(0,0,0,255))
					draw.SimpleText(self.Owner:Name() or "Unknown", "PassHud", 40, -40, Color(0,0,0,255))
					draw.SimpleText("İşletme Türü:", "PassHud2", 40, -17, Color(0,0,0,255))
					draw.SimpleText("Kurumsal", "PassHud3", 40, -5, Color(0,0,0,255))
					draw.SimpleText("Alınacak Vergi:", "PassHud2", 40, 15, Color(0,0,0,255))
					draw.SimpleText("Çalışan başına 1800₺", "PassHud3", 40, 30, Color(0,0,0,255))
		...
		for k, name in pairs( self.wRenderOrder ) do
			...
			elseif (v.type == "Quad") then
				...
				cam.Start3D2D(drawpos, ang, 0.02)
				    draw.SimpleText("Ad-Soyad:", "PassHud2", 40, -50, Color(0,0,0,255))
					draw.SimpleText(self.Owner:Name() or "Unknown", "PassHud", 40, -40, Color(0,0,0,255))
					draw.SimpleText("Meslek:", "PassHud2", 40, -17, Color(0,0,0,255))
					draw.SimpleText(self.Owner:getDarkRPVar("job") or "Unknown", "PassHud3", 40, -5, Color(0,0,0,255))
					draw.SimpleText("Şehir:", "PassHud2", 40, 15, Color(0,0,0,255))
					draw.SimpleText("Nirvana", "PassHud3", 40, 30, Color(0,0,0,255))
					draw.SimpleText("<G<<<<<<<<<<<"..self.Owner:SteamID().."<<<<", "PassHud2", -60, 60, Color(0,0,0,200))
					draw.SimpleText("<G<<<2281337<<<<777<<<<06<<<<<<<<<<", "PassHud2", -60, 75, Color(0,0,0,200))
					surface.SetDrawColor(255,255,255,250)
					surface.SetMaterial(self.mat_ply)
